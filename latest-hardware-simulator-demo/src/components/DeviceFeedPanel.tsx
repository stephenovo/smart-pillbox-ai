...
import type { HardwareDeviceState, IntegrationMode } from "../types/hardware";
...
export function DeviceFeedPanel({
  ...
  useEffect(() => {
    ...
    const selectedDeviceId = activeDeviceId;
    ...
    async function syncDeviceState() {
      try {
        const response = await fetch(
          `/api/hardware/state?deviceId=${encodeURIComponent(selectedDeviceId)}`,
          { cache: "no-store" }
        );
  ...
  async function setReminder(status: "idle" | "reminding") {
    if (!activeDeviceId) {
      setControlError("Select Simulator or Hardware before sending a reminder.");
      return;
    }
  ...
  const sourceEvents = activeDeviceId
    ? events.filter((event) => event.deviceId === activeDeviceId)
    : [];
  const simulationEvents = events.filter((event) => event.source === "simulation");
  const latestSourceEvent = sourceEvents[0];
  const visibleDeviceState =
    deviceState?.deviceId === activeDeviceId ? deviceState : null;
  const isConnected = visibleDeviceState?.connectionStatus === "connected";

  return (
    <div className="space-y-7">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase text-teal-700">Input adapter</p>
        ...
        >
          ...
          {integrationMode === "standalone"
            ? "Standalone"
            : getConnectionLabel(visibleDeviceState)}
        </div>
      </div>
      ...
      <section className="grid overflow-hidden rounded-lg border border-stone-200 bg-white sm:grid-cols-3 sm:divide-x sm:divide-stone-100">
        <div className="flex items-center gap-4 border-b border-stone-100 p-5 sm:border-b-0">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 truncate font-bold text-neutral-950">
              {visibleDeviceState?.deviceId ?? activeDeviceId ?? "No source selected"}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-4 border-b border-stone-100 p-5 sm:border-b-0">
          ...
          <div>
            <p className="text-xs text-neutral-500">Selected source</p>
        ...
        <div className="flex items-center gap-4 p-5">
          ...
          <div className="min-w-0">
            <p className="text-xs text-neutral-500">Latest source event</p>
            <p className="mt-1 truncate font-bold text-neutral-950">
              {latestSourceEvent?.eventTime ?? "No event received"}
            </p>
          </div>
        </div>
      </section>
        ...
        <aside className="space-y-5">
          <section className="rounded-lg border border-stone-200 bg-white p-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-neutral-950">Source reminder</h2>
            </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <button
                ...
                disabled={controlPending || !activeDeviceId}
            ...
            <dl className="mt-5 divide-y divide-stone-100 border-y border-stone-100 text-sm">
              <div className="flex justify-between gap-3 py-3">
                ...
                <dd className="font-semibold text-neutral-950">
                  {visibleDeviceState?.status === "reminding"
                    ? `Reminding Slot ${visibleDeviceState.activeSlot}`
                    : "Idle"}
                </dd>
              </div>
              <div className="flex justify-between gap-3 py-3">
                ...
                <dd className="font-semibold capitalize text-neutral-950">
                  {visibleDeviceState?.trigger ?? "None"}
                </dd>
              </div>
            </dl>
